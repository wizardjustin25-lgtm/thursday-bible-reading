...
export default {
  async fetch(request:Request,env:QuestionEnv,ctx:ExecutionContext){
    ...
    if(origin===allowed){headers.set("Access-Control-Allow-Origin",allowed);headers.set("Access-Control-Allow-Methods","GET, POST, PATCH, DELETE, OPTIONS");headers.set("Access-Control-Allow-Headers","Content-Type, Authorization");headers.set("Access-Control-Max-Age","3600");}
    if(origin&&origin!==allowed)return new Response("Forbidden",{status:403,headers});
    const path=new URL(request.url).pathname;
    if(path!=="/api/questions"&&path!=="/health"&&!path.startsWith("/api/admin/"))return new Response("Not found",{status:404,headers});
    ...
    let response:Response;try{response=path.startsWith("/api/admin/")?await adminApi(request,env):await questionsApi(request,{...env,ALLOWED_ORIGIN:allowed},ctx);}catch{response=Response.json({error:"요청을 처리하지 못했습니다."},{status:400});}
